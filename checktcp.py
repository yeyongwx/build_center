...
def checkserver(sock):
    ...
    sock.close()
    import os
    chkfile = os.path.join('status','tcpserveron.txt')
    count = 0
...
while (count < 5):
    try:
        ...
        count = count +1
        print "[%s] Connection failed, the server is not online. Sleep 10 seconds then retry..."%(time.strftime("%Y-%m-%d %H:%M:%S"))
        time.sleep(10)
